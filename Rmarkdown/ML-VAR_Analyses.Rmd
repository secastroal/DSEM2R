...
Turn response times into hours, the first observation of each subject is hour 1, from then on 
...
for (i in 1:length(ids)) {
  ...
  print(ids[i])
  esmpack::expand.esm(tmp, id = "id", time = "time", tinterval = 1)
}
rm(tmp)
```
...
```{r}
setwd("Mplus/")
data_mlvar_tmp <- data_mlvar[, c("lifepak_id", "response_no_night", "interest", "sad")]
data_mlvar_tmp <- na.omit(data_mlvar_tmp)
names(data_mlvar_tmp) <- c("id", "time", "int", "sad")
prepareMplusData(data_mlvar_tmp, filename = "mlvar.dat", inpfile = TRUE)
```

```{r}
setwd("Mplus/")
runModels("mlvar.inp")
mlvarfit <- readModels("mlvar.out")
```
