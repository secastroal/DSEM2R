...
for (i in 1:length(ids)) {
  tmp <- data_var[data_var$lifepak_id == ids[i], 
                  c("response_no", "interest", "sad")]
  tmp <- na.omit(tmp)
  names(tmp) <- c("beep", "int", "sad")
  
  varfit[[i]] <- var2Mplus(y = c("int", "sad"), x = "beep", data = tmp,
                           beta.at.0 = c(FALSE, TRUE),
...
results <- lapply(varfit, function(x) {
  out <- cbind(paste0("par", 1:9), 
               x$parameters$unstandardized[c(1:3, 5:7, 9, 12, 13), 
                                           c(3, 6, 7)])
  return(out)
})
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par1", ]
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par2", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par3", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par4", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par5", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", 1:9)` == "par6", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", c(1:2, 4:5, 7:9))` == "par2", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", c(1:2, 4:5, 7:9))` == "par4", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
```{r}
tmp <- results[results$`paste0("par", c(1:2, 4:5, 7:9))` == "par5", ]
tmp <- tmp[order(tmp$est), ]
plot(tmp$est, ylim = c(-1, 1))
segments(x0 = 1:53, 
         y0 = tmp$lower_2.5ci,
...
for (i in 1:length(ids)) {
  ...
  tmp$time <- (tmp$time - tmp$time[1])/3600 
